...
resource "azurerm_private_dns_zone" "kpa23-pdns-dg" {
  name                = "dgwebapp.com"
  resource_group_name = azurerm_resource_group.kpa23-rg-dg.name
}

resource "azurerm_private_dns_zone_virtual_network_link" "kpa23-vnlink-dg" {
...
resource "azurerm_virtual_network" "kpa23-vn-dg" {
  name                = "kpa23-vn-dg"
